...
def create_timeseries_store(catalog: Catalog, store_name: str, zip_path: str, workspace=None, layer_name=None, srs=None, footprint='Transparent', accurate_resolution_computation=True):
    if layer_name == None:
    ...
    # prepare parmeters for next steps
    ...
    if accurate_resolution_computation:
        logging.info(f'Setting Accurate Resolution Computation to {accurate_resolution_computation}')
        accurate_resolution_computationEntry = list(filter(lambda entry: entry['string'][0] == 'Accurate resolution computation', parameters['entry']))[0]
        accurate_resolution_computationEntry['string'][1] = accurate_resolution_computation
...
def publish_timeseries(workspace_name, store_name, layer_name, zip_path, style_data = None, srs = None, footprint = 'Transparent', style_name = None, overwrite = True, accurate_resolution_computation = False):
    catalog = get_catalog()
    ...
    store = create_timeseries_store( 
        ...
        srs = srs,
        footprint = footprint,
        accurate_resolution_computation=accurate_resolution_computation
    )
...
def create_single_granule_zip(file_name, path):
    ...
    # file_name is the specifc file to zip
